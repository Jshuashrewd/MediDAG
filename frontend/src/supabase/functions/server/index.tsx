...
app.put(
  "/make-server-140653bc/user/notifications/:notificationId/read",
  async (c) => {
    const { error, user } = await verifyAuth(c.req.header("Authorization"));
    if (error || !user) {
      return c.json({ error: "Unauthorized" }, 401);
    ...
    const tasks = [
      {
        ...
        coins: 50,
        completed: false,
        description: "Fill out a brief health survey",
      },
      {
        id: "2",
        ...
        coins: 100,
        completed: false,
        description: "Upload your previous medical records",
      },
      {
        id: "3",
      ...
      {
        id: "5",
        title: "Complete Medication Adherence",
        coins: 150,
        completed: false,
        description: "Track your medications for 30 days",
      },
    ];
...
app.get(
  "/make-server-140653bc/hospital/patients/search/:patientId",
  async (c) => {
    const { error, user } = await verifyAuth(c.req.header("Authorization"));
    if (error || !user) {
      return c.json({ error: "Unauthorized" }, 401);
    ...
    try {
      ...
      const demoPatient = demoPatients.find(
      ...
      const patient = allUsers.find(
        (u: any) => u.value?.patientId === patientId
      );
      ...
      const reports =
        (await kv.get(`medical-reports:${patient.value.id}`)) || [];
...
app.post(
  "/make-server-140653bc/hospital/patients/:patientId/update",
  async (c) => {
    const { error, user } = await verifyAuth(c.req.header("Authorization"));
    if (error || !user) {
      return c.json({ error: "Unauthorized" }, 401);
    ...
    try {
      ...
      const demoPatients = (await kv.get(`demo-patients:${user.id}`)) || [];
      const isDemo = demoPatients.some(
        (p: any) => p.patientId === patientIdParam
      );
      ...
      const allUsers = await kv.getByPrefix("user:");
      const patient = allUsers.find(
        (u: any) => u.value?.patientId === patientIdParam
      );
      ...
      const notifications =
        (await kv.get(`notifications:${patientUserId}`)) || [];
      const notification = {
        id: pendingReport.id,
        type: "verification",
        message: `${doctorName || "A doctor"} from ${
          hospitalData?.hospitalName
        } wants to update your medical record. Please verify.`,
        data: {
      ...
      return c.json(
        { error: `Failed to update patient data: ${err.message}` },
        500
      );
    }
  }
